...
- 实现终端交互基本界面与逻辑
  并在终端实时打印 Inquirer/医生问题与患者回答、分诊结果及阶段进度
"""
...
try:
    ...
    # 引入 StepExecutor class 以便注入带 true_patient 的实例
...
def interactive_patient_run_factory():
    """
    返回一个 callable 用以作为 true_patient_agent 的实现 (具有 .run 接口)。
    run 的签名： (worker_inquiry: str, is_first_epoch: bool, patient_case: dict) -> object-with-current_chat
    """
...
# 轻量包装器：将 interactive_run（函数）包装为具有 .run 方法的对象，供 StepExecutor 注入
class TerminalTruePatient:
    def __init__(self, runner: Callable[[str, bool, dict], Any]):
        self._runner = runner

    def run(self, worker_inquiry: str = "", is_first_epoch: bool = False, patient_case: dict = None):
        return self._runner(worker_inquiry, is_first_epoch, patient_case)


def print_round_overview(step: int, task_manager, conversation_history: str):
...
def run_interactive_case(case_data: Dict[str, Any], args: argparse.Namespace, case_index: int = 0):
    ...
    # 准备交互 runner（作为 true_patient_agent 注入 StepExecutor）
    interactive_runner_func = interactive_patient_run_factory()
    # 包装为对象（带 .run 方法），以兼容 StepExecutor 对 true_patient 的期望
    terminal_true_patient = TerminalTruePatient(interactive_runner_func)
    ...
    try:
        step_executor = StepExecutor(
            ...
            controller_mode="normal",
            guidance_loader=loader,
            true_patient_agent=terminal_true_patient
        )
    except Exception as e:
        raise RuntimeError(f"无法创建 StepExecutor 并注入交互 runner：{e}")

    # 使用我们创建的 step_executor 替换 workflow 内部的实例
    workflow.step_executor = step_executor
    ...
    print("交互说明：在提示时输入患者回答；输入 :exit/:quit/:q 以退出。")
    print("#" * 60 + "\n")

    # 手动控制主循环（替代调用 workflow.run），以便在每轮后即时打印 Inquirer/患者对话与分诊结果
    ...
    try:
        for step in range(1, args.max_steps + 1):
            ...
            # 打印轮次概览（按图片要求）
            print_round_overview(step, task_manager, conversation_history)
            ...
            # 更新本地工作流状态（与 MedicalWorkflow._update_workflow_state 保持一致）
            ...
            # 将医生问题存回 workflow 以便下一轮使用
            workflow._last_doctor_question = step_result.get("doctor_question", "")
            ...
            for phase_name, phase_info in completion_summary["phases"].items():
                ...
                print(f"  {status} {phase_name}: {phase_info['completed']}/{phase_info['total']} ({phase_info['completion_rate']:.1%})")
            # 患者与医生对话（本轮）
            print(f"患者: {step_result.get('patient_response','')}")
            print(f"医生: {step_result.get('doctor_question','')}")
            # 分诊结果
            ...
            if tr:
                ...
                if tr.get("triage_reasoning"):
                    print(f"  分诊理由: {tr.get('triage_reasoning')[:300]}")
            print("----------------------\n")
