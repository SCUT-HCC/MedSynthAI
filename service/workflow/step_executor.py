#变更说明：
#- 移除对 VirtualPatientAgent的导入。
#- 移除virtual_patient相关模块，支持通过 true_patient_agent 注入虚拟患者实现
#  如果未注入 true_patient_agent，则在运行时提示并允许终端再次输入以继续当前轮次。
#- 保留 [Inquirer]和分诊相关的逻辑
import time
import sys
import os
...
# 取消在此处直接导入 agent_system.virtual_patient
...
class StepExecutor:
    ...
    def __init__(self, model_type: str = "deepseek", 
                llm_config: dict = None, 
                 ...
                 true_patient_agent: Optional[Any] = None,
                ):
        """
        初始化step执行器
        ...
        Args:
            ...
            true_patient_agent: 可选的虚拟患者实现（推荐注入；需包含 .run 接口）。
                如果不提供，运行时会提示并允许终端人工输入以继续当前轮次。
        ...
        # 虚拟患者实现：优先使用注入的 true_patient_agent；若为 None，则在运行时使用终端人工输入作为回退
        self.true_patient = true_patient_agent
    ...
    def _get_patient_response(self, step_num: int, case_data: Dict[str, Any], 
        ...
        """获取患者的回应；若未注入 true_patient，则提示并允许终端再次输入以继续当前轮次"""
        ...
        try:
            ...
            # 如果未注入 true_patient，实现回退：在终端提示用户直接输入患者回答（以便“重新当前轮次问询”）
            if self.true_patient is None:
                ...
                logger.log_agent_execution(
                    step_num, "true_patient",
                    {"worker_inquiry": worker_inquiry, "is_first_epoch": is_first_step},
            ...
            # 调用注入的 true_patient 实现
            patient_result = self.true_patient.run(
            ...
            error_msg = f"true_patient 执行失败: {str(e)}"
            logger.log_error(step_num, "true_patient_error", error_msg)
            # 返回默认回应
